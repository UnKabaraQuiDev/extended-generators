...
public class CmdConfigIsland implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 1) {
			if (args[0].equalsIgnoreCase("set")) {
				...
				player.sendMessage(ChatColor.GOLD + "Set island spawn point to: " + ChatColor.GREEN
			...
			if (args[0].equalsIgnoreCase("blacklist") && args[1].equalsIgnoreCase("list")) {
				...
				sender.sendMessage(
						ChatColor.GOLD + "Blacklist: " + ChatColor.GREEN + String.join(", ", pd.getBlacklist().stream()
								.map(a -> PlayerManager.getPlayer(UUID.fromString(a)).getName()).toList()));

				return true;
			} else {
				sender.sendMessage(ChatColor.RED + "Option " + ChatColor.GREEN + args[1] + " not found ("
						+ ChatColor.GOLD + "add" + ChatColor.RED + ", " + ChatColor.GOLD + "remove" + ChatColor.RED
			...
			if (args[0].equalsIgnoreCase("blacklist")) {
				if (!PlayerManager.knowsPlayer(args[2])) {
					sender.sendMessage(
							ChatColor.RED + "Player: " + ChatColor.GOLD + args[2] + ChatColor.RED + ", unknown");
				...
				if (args[1].equalsIgnoreCase("add")) {
					...
					sender.sendMessage(ChatColor.GOLD + "Player: " + ChatColor.GREEN + args[2] + ChatColor.GOLD
							+ " added to blacklist !");
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
		if (args.length <= 1) {
			return List.of("set", "spawn", "blacklist");
		}
